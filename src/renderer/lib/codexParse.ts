...
export function parseCodexStream(raw: string): ParsedCodexOutput {
  ...
  const isHeader = (s: string) =>
    ...
    s === '--------' ||
    /^(workdir|model|provider|approval|sandbox|reasoning effort|reasoning summaries)\s*:/i.test(s) ||
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (section === 'reasoning') {
      ...
      const pronounMeta = /^(?:I['’]m|I am|I['’]ll|I will|Let['’]s|Let us|We['’]ll|We will|We['’]re|We are|I need to|I plan to|I['’]m going to)\b/i.test(t)
      const gerundMeta = /^(Exploring|Analyzing|Investigating|Planning|Considering|Assessing|Reviewing|Providing)\b/i.test(t)
      ...
      const substantiveHeuristic = (hasWordChars && !isMeta) || looksCodeFence || looksList || looksHeading
