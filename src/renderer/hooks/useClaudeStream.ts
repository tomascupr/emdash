...
interface Options { workspaceId: string; workspacePath: string }
...
const useClaudeStream = (options?: Options | null): Result => {
  ...
  useEffect(() => {
    if (!isStreaming) { if (timerRef.current) clearInterval(timerRef.current); return }
  ...
  const send = useCallback(async (text: string, attachments: string = '') => {
    ...
    try {
      await window.electronAPI.saveMessage({ id: userMessage.id, conversationId: convoId, content: userMessage.content, sender: userMessage.sender })
    } catch {}
    appendMessage(userMessage)

    setIsStreaming(true); setSeconds(0); bufferRef.current = ''; setStreamingOutput(''); setAwaitingThinking(true)
    try {
      await window.electronAPI.agentSendMessageStream({ providerId: 'claude', workspaceId: normalized.workspaceId, worktreePath: normalized.workspacePath, message: `${text}${attachments ?? ''}`, conversationId: convoId })
      return { success: true }
    } catch (e: any) {
      setIsStreaming(false); setSeconds(0)
  ...
  const cancel = useCallback(async () => {
    ...
    try {
      const res = await window.electronAPI.agentStopStream({ providerId: 'claude', workspaceId: normalized.workspaceId })
  ...
  useEffect(() => {
    let cancelled = false
    if (!normalized) { setIsLoading(false); setIsReady(false); setMessages([]); setStreamingOutput(''); setIsStreaming(false); setSeconds(0); return () => { cancelled = true } }
      ...
      try {
        ...
        if (convo?.success && convo.conversation) {
          ...
          if (msgs?.success && Array.isArray(msgs.messages)) {
            setMessages(msgs.messages.map((m: any) => ({ id: m.id, content: m.content, sender: m.sender, timestamp: new Date(m.timestamp) })))
          } else {
            setMessages([])
          }
        } else {
  ...
  return { messages, conversationId, isLoading, isReady, streamingOutput, isStreaming, awaitingThinking, seconds, send, cancel, appendMessage }
}
