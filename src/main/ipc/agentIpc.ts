...
export function registerAgentIpc() {
  ...
  ipcMain.handle('agent:get-installation-instructions', async (_e, providerId: 'codex' | 'claude') => {
    try {
  ...
  // Start streaming
  ipcMain.handle('agent:send-message-stream', async (_e, args: { providerId: 'codex' | 'claude'; workspaceId: string; worktreePath: string; message: string; conversationId?: string }) => {
    try {
  ...
  // Stop streaming
  ipcMain.handle('agent:stop-stream', async (_e, args: { providerId: 'codex' | 'claude'; workspaceId: string }) => {
    try {
  ...
  codexService.on('codex:output', (data: any) => {
    ...
    windows.forEach((w) => w.webContents.send('agent:stream-output', { providerId: 'codex', ...data }))
  })
  codexService.on('codex:error', (data: any) => {
    const windows = BrowserWindow.getAllWindows()
    windows.forEach((w) => w.webContents.send('agent:stream-error', { providerId: 'codex', ...data }))
  })
  codexService.on('codex:complete', (data: any) => {
    const windows = BrowserWindow.getAllWindows()
    windows.forEach((w) => w.webContents.send('agent:stream-complete', { providerId: 'codex', ...data }))
  })
